...
class MotionSynthesizer():
    ...
    def compute_norms(self, reference, current_features, coefficient):
        ...
        for i in range(len(current_features)):
            norms.append(np.linalg.norm(reference - (current_features[i] * coefficient[i])) ** 2)
        ...
        for i in range(len(self._mocap)):
            ...
            if(i % self._extraction_framerate == 0):
                ...
                if(coefficient > 1.0):
                    ...
                    current_root_height = self._mocap[i]["root"][0][1]
                    new_root_height = current_root_height
                    new_root_height += (-current_root_height * (coefficient - 1.0))
    ...
    def rule_2(self):
        ...
        dt_head = (t_pose_root[0] - t_pose_head[0], t_pose_root[1] - t_pose_head[1], t_pose_root[2] - t_pose_head[2])
        ...
        for i in range(len(self._mocap)):
            ...
            if(i % self._extraction_framerate == 0):
                ...
                d_head = (current_root_position[0] - current_neck_position[0], current_root_position[1] - current_neck_position[1], current_root_position[2] - current_neck_position[2])
                ...
                new_neck_position_x += dt_head[0] - ((dt_head[0]-d_head[0])/coefficient) - d_head[0]
                new_neck_position_y += dt_head[1] - ((dt_head[1]-d_head[1])/coefficient) - d_head[1]
                new_neck_position_z += dt_head[2] - ((dt_head[2]-d_head[2])/coefficient) - d_head[2]

                gen_index = next(index for index in range(len(self.generated_mocap)) if self.generated_mocap[index]["index"] == i)

                print(self.generated_mocap[gen_index]['mocap']['neck'][0])
                self.generated_mocap[gen_index]['mocap']['neck'][0] = (new_neck_position_x, new_neck_position_y, new_neck_position_z)
                print(self.generated_mocap[gen_index]['mocap']['neck'][0])
                print()
    ...
    def rule_3(self):
        ...
        for i in range(len(self._mocap)):
            ...
            if(i % self._extraction_framerate == 0):
                ...
                current_ground_left = (current_left_hand[0], 0.0, current_left_hand[2])
                current_ground_right = (current_right_hand[0], 0.0, current_right_hand[2])

                dl_head = (current_head[0] - current_left_hand[0], current_head[1] - current_left_hand[1], current_head[2] - current_left_hand[2])
                dr_head = (current_head[0] - current_right_hand[0], current_head[1] - current_right_hand[1], current_head[2] - current_right_hand[2])

                dl_chest = (current_chest[0] - current_left_hand[0], current_chest[1] - current_left_hand[1], current_chest[2] - current_left_hand[2])
                dr_chest = (current_chest[0] - current_right_hand[0], current_chest[1] - current_right_hand[1], current_chest[2] - current_right_hand[2])

                dl_root = (current_root[0] - current_left_hand[0], current_root[1] - current_left_hand[1], current_root[2] - current_left_hand[2])
                dr_root = (current_root[0] - current_right_hand[0], current_root[1] - current_right_hand[1], current_root[2] - current_right_hand[2])

                dl_ground = (current_ground_left[0] - current_left_hand[0], current_ground_left[1] - current_left_hand[1], current_ground_left[2] - current_left_hand[2])
                dr_ground = (current_ground_right[0] - current_right_hand[0], current_ground_right[1] - current_right_hand[1], current_ground_right[2] - current_right_hand[2])
                ...
                new_left_hand_position_x += (dl_head[0] * (coefficient - 1.0) + dl_chest[0] * (coefficient - 1.0) + dl_root[0] * (coefficient - 1.0) + dl_ground[0] * (coefficient - 1.0))/4.0
                new_left_hand_position_y += (dl_head[1] * (coefficient - 1.0) + dl_chest[1] * (coefficient - 1.0) + dl_root[1] * (coefficient - 1.0) + dl_ground[1] * (coefficient - 1.0))/4.0
                new_left_hand_position_z += (dl_head[2] * (coefficient - 1.0) + dl_chest[2] * (coefficient - 1.0) + dl_root[2] * (coefficient - 1.0) + dl_ground[2] * (coefficient - 1.0))/4.0

                new_right_hand_position_x += (dr_head[0] * (coefficient - 1.0) + dr_chest[0] * (coefficient - 1.0) + dr_root[0] * (coefficient - 1.0) + dr_ground[0] * (coefficient - 1.0))/4.0
                new_right_hand_position_y += (dr_head[1] * (coefficient - 1.0) + dr_chest[1] * (coefficient - 1.0) + dr_root[1] * (coefficient - 1.0) + dr_ground[1] * (coefficient - 1.0))/4.0
                new_right_hand_position_z += (dr_head[2] * (coefficient - 1.0) + dr_chest[2] * (coefficient - 1.0) + dr_root[2] * (coefficient - 1.0) + dr_ground[2] * (coefficient - 1.0))/4.0

                gen_index = next(index for index in range(len(self.generated_mocap)) if self.generated_mocap[index]["index"] == i)

                print(self.generated_mocap[gen_index]['mocap']['left_wrist'][0])
                self.generated_mocap[gen_index]['mocap']['left_wrist'][0] = (new_left_hand_position_x, new_left_hand_position_y, new_left_hand_position_z)
                self.generated_mocap[gen_index]['mocap']['right_wrist'][0] = (new_right_hand_position_x, new_right_hand_position_y, new_right_hand_position_z)
                print(self.generated_mocap[gen_index]['mocap']['left_wrist'][0])
                print()
...
ms = MotionSynthesizer("neutral_walk_mocap.txt", "neutral_walk_lma.txt", SAD_LMA)
